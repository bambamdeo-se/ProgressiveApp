(function ($) {
	$.fn.imageUploaderCut = function (options) {
		var innerOptions = {
			...
			pluginObject: this,
			apertureWidth: options.apertureWidth || 48,
			apertureHeight: options.apertureHeight || 48,
			proceedCallBack: options.proceedCallBack
		};
		this.append('<p><input type="file" name="files" value="" accept="image/*.jpg" id="' + innerOptions.fileInputId + '"/></p>');		
		this.append('<div id="cropContainer"></div>');
		this.append('<p><input type="button" name="getCroppedImagesJson" value="Set Profile Picture"/></p>');
